...
\keywords{Vehicle Routing Problem, Black-box optimization, Genetic Algorithms, Probability Collectives, Distributed Systems, \hfcomment{\& more - user interfaces, }}
...
HGSDAC evolves infeasible and feasible solutions as two separate subpopulations. Two parents are selected by a fitness function \hfcomment{binary tournament} iteratively applied to the (both!) population, and then a crossover operator creates an child. A local search is then applied to the child, and then may be repaired \hfcomment{explain or skip} and placed into an appropriate subpopulation. If a subpopulation exceeds a maximum size, a survivor selection stage is triggered \hfcomment{explain...}. A diversification round may be started if there has been $It_{div}$ iterations without improvement. \hfcomment{to be cont, pending decisions on what to gloss over.} 
